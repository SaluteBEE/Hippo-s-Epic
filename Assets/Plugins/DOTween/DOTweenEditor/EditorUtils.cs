...
//        }
        ...
        static void StoreDOTweenDirsAndFilePaths()
        {
            ...
            // 用 AssetDatabase 查找 DOTweenEditor 目录，避免无 asmdef 时路径推算错误
            var editorGuids = AssetDatabase.FindAssets("EditorUtils t:Script", new[] { "Assets/Plugins/DOTween" });
            if (editorGuids.Length > 0) {
                var editorScriptPath = AssetDatabase.GUIDToAssetPath(editorGuids[0]);
                _editorDir = Path.GetDirectoryName(Path.GetFullPath(editorScriptPath));
            } else {
